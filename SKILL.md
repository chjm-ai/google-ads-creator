...
description: Google Ads 投放助手，支持三个独立任务：(1) 通过问答引导生成投放计划文档；(2) 根据投放计划生成新建广告所需的 Google Ads Editor 可导入 CSV；(3) 基于用户从 Google Ads/Google Ads Editor 导出的现有数据，生成更新现有广告所需的 CSV。Use when the user wants to create Google Ads campaigns, generate ad import files, update existing campaigns in bulk, prepare search ads, plan ad strategy, or says things like "帮我生成投放文件", "帮我更新投放CSV", "create google ads campaign", "update google ads csv", "生成谷歌广告", "准备投放素材", "制定投放计划", "plan my ad campaign".
---

# Google Ads Campaign Creator

本 skill 提供三个独立任务，可以按顺序执行，也可以单独运行：

| 任务 | 说明 | 输出 |
|------|------|------|
| **任务一：生成投放计划** | 通过问答引导，收集用户业务信息，AI 生成完整投放策略文档 | `投放计划.md` |
| **任务二：生成新建导入文件** | 根据投放计划，生成“首次创建广告”所需的 Google Ads Editor 可导入 CSV 文件 | 5 个 CSV 文件 |
| **任务三：生成更新文件** | 基于用户从 Google Ads 网页版或 Google Ads Editor 导出的现有数据，生成“更新现有广告/预算/文案/URL”所需的 CSV 文件 | `*_update.csv` 或基于原文件改写后的更新 CSV |
...
用户说了什么？
├── 已有完整投放计划 → 直接进入「任务二」
├── 已有 Google Ads 导出数据，想修改后再上传 → 进入「任务三」
├── 说“更新 CSV / 更新广告 / 改预算后重新上传”但没有导出数据 → 先让用户去导出，再进入「任务三」
├── 只有产品/业务信息，没有投放计划 → 从「任务一」开始
├── "帮我制定投放计划" / "plan my campaign" → 「任务一」
├── "帮我生成导入文件" / "generate CSV" → 「任务二」（检查是否有计划）
├── "帮我更新导入文件" / "update CSV" / "refresh bulk upload" → 「任务三」（检查是否有导出数据）
└── 不确定 → 询问用户："你是需要我先帮你制定投放计划，还是已经有计划只需要生成导入文件？"
```

**新增判断规则：**
- 用户说“后续更新”“重新上传会报错”“改预算/URL/文案后再导入”，默认优先判断为「任务三」
- **严禁**拿“首次创建用 CSV”直接当“更新 CSV”使用，除非用户明确说是首次创建
- 任务三里，若用户没有提供从 Google Ads/Editor 导出的现有数据，必须先提示用户去导出；**不要**凭空生成所谓“更新模板”去覆盖线上已有广告
...
当用户已经在 Google Ads 里建好了 Campaign / Ad Group / Keyword / Ad，只是想：

- 更新预算、状态、出价
- 更新 Final URL、Tracking Template、Labels
- 更新关键词出价或暂停状态
- 基于现有广告改文案再重新上传

这类都应走「任务三」，而不是重复生成任务二的“新建 CSV”。

## 核心原则

**更新现有广告时，必须基于 Google Ads/Google Ads Editor 导出的现有数据进行修改。**

原因：
- 更新时通常需要 `Campaign ID`、`Ad group ID`、`Ad ID`、`Keyword ID` 或 `#Original` 列
- 这些字段用户手工编不出来，也不应猜
- 直接拿新建模板改内容再上传，系统容易把它识别成“新建”或因为缺少 ID 直接报错
...
| 必需信息 | 来源 | 缺失时的提示 |
|----------|------|-------------|
| **Google Ads 导出文件** | 用户从 Google Ads 网页版或 Google Ads Editor 导出 | "更新现有广告时，我需要你先从 Google Ads 或 Google Ads Editor 导出当前数据，再把导出的 CSV/XLSX 发给我。不能直接拿首次创建用的模板来更新。" |
| **更新目标** | 用户说明 | "请告诉我你要更新什么：预算、出价、关键词、文案、URL、状态，还是其他字段？" |
| **作用范围** | 用户说明 / 导出文件中识别 | "请告诉我是更新全部 Campaign，还是只更新某几个国家/广告组/关键词。" |

## 必须提示用户如何导出

如果用户还没有导出数据，必须明确提示以下操作：

### 方式 A：Google Ads 网页版
1. 打开 Google Ads
2. 进入要更新的对象页面（广告系列 / 广告组 / 关键词 / 广告）
3. 点击右上角下载图标
4. 选择“更多选项”
5. 勾选“用于批量上传的可修改列”
6. 下载为 CSV 或 Excel

### 方式 B：Google Ads Editor
1. 打开 Google Ads Editor 并同步账户
2. 进入要更新的对象视图
3. 导出当前视图到 CSV
4. 如可选，优先保留 `#Original` 列，便于后续精准匹配更新

**提示语模板：**

> 你这是“更新现有广告”，不是“首次创建”。请先从 Google Ads 网页版导出带“用于批量上传的可修改列”的文件，或者从 Google Ads Editor 导出当前数据；拿到导出文件后我再帮你生成更新版 CSV。否则直接改创建模板，重新上传大概率会报错或被识别成新建。
...
1. **保留原始标识列**
   - 不删除 `Campaign ID`、`Ad group ID`、`Ad ID`、`Keyword ID`、`#Original` 等列
2. **仅修改用户要求更新的字段**
   - 未要求改动的列尽量保持原值
3. **不要把更新误改成新建**
   - 如果导出行里已有 ID，通常不要把 `Action` 改成 `Add`
   - 需要时根据原文件语义使用 `Edit`
4. **广告文案更新要特别谨慎**
   - 如果用户要改 RSA 文案，优先基于 Google Ads Editor 导出的广告文件处理
   - 不要用任务二的新建广告 CSV 去“覆盖更新”现有广告
5. **输出文件命名**
   - 建议命名为 `campaign_update.csv`、`keyword_update.csv`、`responsive_search_ad_update.csv`
   - 或者在原文件名基础上增加 `_update`
...
- 本次修改了哪些字段
- 这些文件基于哪份官方导出数据改写
- 建议先在 Google Ads Editor 预览变更，再发布
...
- **不要删除 ID 列**
- **不要把任务二的创建模板当任务三更新模板**
- **如果用户只提供了首次创建 CSV，却说要更新线上内容，先纠正流程，再继续**
